...
### Added
- `examples/demo-dashboard.html`: a five-step walkthrough of the same analysis — the verdict, the
  cost curve with a live threshold, the four questions measured separately, the segments, and the
  drift gate failing on a degraded candidate model — in one file, opened locally, walked with the
  arrow keys, with a narration line per step for recording. It is not a product surface: it is
  generated by `examples/make-demo-dashboard.py`, which calls the same library the report does, so
  every figure on it is computed rather than typed in, and `tests/test_demo_dashboard.py` rebuilds
  the file and requires a byte-for-byte match.
