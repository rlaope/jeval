...
def format_ci_block(
    ...
    check and its numbers, the paired head-to-head block when one is given, the view's note when
    there is one, and the exit code the block implies. The paired block informs; it never changes
    the exit code, which only the ``--fail-on`` checks decide.
    """
